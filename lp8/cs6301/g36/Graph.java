...
public class Graph implements Iterable<Graph.Vertex> {
    ...
    public static class Vertex implements Iterable<Edge> {
        ...
        Iterator<Graph.Edge> it; // add by xintong for euler tour finding
        List<Edge> subtours = new LinkedList<>(); // add by xintong for euler tour finding
        boolean visited =false; //add for SCC
        boolean isTourExplored; // add by XinTong for euler tour finding
        int componentId=-1;
        Edge predecessor; //
        int distance=Integer.MAX_VALUE; //distance from source
        int count = 0; //how many times the vertex has been processed
